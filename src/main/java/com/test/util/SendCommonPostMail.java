...
public class SendCommonPostMail {
    ...
    public static int  Send(String email,int type) {
        ...
        params.add(new BasicNameValuePair("html", htmltext(email,type)));
        try {
    ...
    private static String htmltext(String email,int type){
        ...
        if(type == 1){
            html =
                    "<style type=\"text/css\">html,\n" +
                            ...
                            "<h1>这是一个测试页面</h1><br/>"+
                            "<a href='http://localhost:8080/qgshare/user/" + ciphertext+"/form?email=" +email+ "'>点击前往注册界面</a></p>\n" +
                            "\t\t\t\t\t\t\t\t\t\t\t\t</div>\n" +
                            "\n" +
